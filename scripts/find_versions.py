...
"""
    ...
    @staticmethod
    def download_bucket_key(download: PythonDownload) -> tuple[Version, bool]:
        return download.version, "freethreaded" in download.filename
    ...
    async def fetch_indygreg_downloads(self, pages: int = 100) -> list[PythonDownload]:
        ...
        results: dict[tuple[Version, bool], dict[VersionKey, list[PythonDownload]]] = {}
            ...
            for row in rows:
                for asset in row["assets"]:
                    ...
                    if download is not None:
                        ...
                        bucket_key = self.download_bucket_key(download)
                        key = VersionKey(
                        ...
                        (
                            ...
                            # Freethreaded builds must remain separate or they can replace GIL builds with the same flavor.
                            .setdefault(key, [])
                            .append(download)
                        )
    ...
    def pick_best_download(cls, downloads: list[PythonDownload]) -> PythonDownload | None:
        ...
        def preference(download: PythonDownload) -> int:
            if download.triple.flavor is None:
...
def render(downloads: list[PythonDownload], file: IO[str] | None = None):
    ...
    def version_sort_key(
        item: tuple[
            tuple[PythonImplementation, Version, bool],
            dict[VersionKey, tuple[str, str | None]],
        ],
    ) -> tuple[int, int, int, str, bool]:
        key, _ = item
        return (-key[1].major, -key[1].minor, -key[1].patch, key[0], key[2])

    payload = []
    for key, item in sorted(build_map(downloads).items(), key=version_sort_key):
        files = []
        for version_key, (url, sha256) in sorted(item.items()):
            files.append(
                {
                    "platform": version_key.platform,
                    "arch": version_key.arch,
                    "install_only": version_key.install_only,
                    "filename": unquote(url.rsplit("/", maxsplit=1)[-1]),
                    "url": url,
                    "sha256": sha256,
                }
